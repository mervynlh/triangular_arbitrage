...
def triangular_arbitrage(q_list, price_list, param_list=[0.0, 0.0], slippage_list=[0.0, 0.0, 0.0], fee_list=[0.002, 0.002, 0.002], coin='end'):
    """
    example:
    htusdt - hteth*ethusdt
    :param q_list: q1代表ht的数量，q2代表eth的数量，q3代表usdt的数量 保证 q1*p1<=q3
    :param price_list:  p1代表ht/usdt的价格，p2代表hteth的价格，p3代表ethusdt的价格
    :param param_list: param_1 代表 阀值，param_2 代表 htusdt begin ht 的交易个数
    :param slippage_list: 滑点list
    :param fee_list: 手续费list
    :param coin: begin mid end ，分别代表三个币的位置 htusdt hteth ethusdt  ht-begin eth-mid usdt-end
    :return: transaction-0 ht/usdt交易的个数，transaction-1 hteth交易的个数，transaction-2 ethusdt交易的个数
    """
    flag = 1
    transaction = []
    p1 = price_list[0]
    ...
    q1 = param_list[1]
    param1 = param_list[0]
    sli_1 = slippage_list[0]
    sli_2 = slippage_list[1]
    sli_3 = slippage_list[2]
    fee_1 = fee_list[0]
    fee_2 = fee_list[1]
    fee_3 = fee_list[2]
    ...
    if positive > param1:
        ...
        p1 = price_list[0] * (1 - sli_1) * (1 - fee_1)
        p2 = price_list[1] * (1 + sli_2) / (1 - fee_2)
        p3 = price_list[2] * (1 + sli_3) / (1 - fee_3)
